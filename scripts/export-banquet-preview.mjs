  ...
  const lines = [
    ...
    ...rows.map((row) => {
      const output = {
        ...
        meal_id: row.meal_id,
        meal: row.meal_name,
        attendee_dietary_or_accommodation_note: row.dietary_note,
        registration_seating_or_group_notes: row.seating_notes,
        checked_in: '',
        table_assignment: '',
        ...
        donation: centsToDollars(row.donation_amount_cents, 'donation_amount_cents'),
        expected_total: centsToDollars(row.expected_total_cents, 'expected_total_cents'),
