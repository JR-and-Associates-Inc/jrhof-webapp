...
Exports default to all statuses; `?paid-only=true` applies the authorized paid-only view. The registration ledger has one row per signup, while the master attendee roster has one row per guest so names, meals, and accommodation notes remain paired. Both protected reports include the stored Checkout Session and PaymentIntent object IDs for Stripe reconciliation. They never include client secrets, API/webhook secrets, PaymentMethod/card data, or raw webhooks. Responses use UTF-8 BOM and CRLF, quote every cell, neutralize spreadsheet formulas, and set `Cache-Control: no-store`, `X-Robots-Tag: noindex, nofollow`, and attachment disposition. Audit rows store only a one-way subject digest, export type, scope, row count, and timestamp. See [Cloudflare Access export setup](../../docs/implementation/CLOUDFLARE_ACCESS_EXPORT.md) and the [board guide](../../docs/operations/BOARD_REGISTRATION_EXPORT_GUIDE.md).
