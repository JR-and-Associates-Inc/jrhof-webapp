...
- **Created the `Inductees - Umpire History` ad group** in `Grants | Brand & Archive` with **10 phrase/exact inductee/umpire-history keywords** (now live and Eligible; campaign total 22 keywords). The accompanying **RSA (12 headlines / 4 descriptions, lands on `/inductees/`) could not be saved** — Google's owner-only **"Confirm it's you" gate hard-blocks ad creation from the automated browser** ("Blocked during authentication"). **TJ must add one RSA** to the ad group from a normal browser (exact copy-paste spec in the execution log §C-update). Until then the ad group has keywords but no ad and won't serve.
