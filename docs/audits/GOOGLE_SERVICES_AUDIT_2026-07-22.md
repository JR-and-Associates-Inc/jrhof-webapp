...
Local runtime validation confirmed the exact Holiday Inn Denver–Lakewood pin, no iframe while the venue remained about 3,155 pixels below the initial viewport, and automatic loading after the venue approached the viewport. The iframe measured about 661 by 380 pixels on desktop and 357 by 300 pixels at a 390-by-844 mobile viewport. It retains a descriptive title, `loading="lazy"`, `referrerpolicy="strict-origin-when-cross-origin"`, and the narrow `frame-src https://www.google.com` CSP. Desktop and mobile checks produced no browser console warnings or errors. JRHOF does not request or collect the visitor's current location. Repeat the same checks against the production URL after deployment.
