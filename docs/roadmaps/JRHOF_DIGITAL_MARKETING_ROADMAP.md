...
- **Status 2026-07-04 (LARGELY EXECUTED):** Three of four campaigns are live and learning — `Grants | Brand & Archive`, `Grants | Donations` (Colorado), and `Grants | Banquet & Community` (Colorado, landing on the 2027 induction-banquet page) — all Search-only, Maximize Clicks $2 cap, AI Max off. Legacy campaigns paused (not deleted). `Shared | Negatives | Core` (25 terms) applied to all three; 6 account-level sitelinks + 5 callouts added. **Remaining:** the seasonal `Grants | Golf` campaign, a 2nd ad group per campaign (currently 1 each), a 2nd RSA on Brand & Archive, and the Brand & Archive budget correction to $120/day. The 2026-07-03 bulk-Sheet upload failed (never shared with the Ads service account) — everything was built via the wizard instead. See `docs/audits/JRHOF_EXECUTION_LOG_2026-07-03_ADS_REBUILD.md` and `…_2026-07-04_ADS_LAUNCH.md`.
...
- **Goal:** Resolve the Zaraz/GTM contradiction, record this architecture as canonical, and remove the obsolete AdSense publisher artifact after JRHOF confirmed it does not use AdSense.
- **Files:** `docs/JRHOF_MASTER_STATUS.md`, `docs/DECISIONS.md`, `docs/launch/SEO_AND_AD_GRANTS_READINESS.md`, `docs/ANALYTICS.md`, and the removed public publisher artifact.
- **Acceptance:** Current operating docs name GTM as the single Google loader, historical audit references remain dated, and current docs state that JRHOF does not use AdSense. Google Ad Grants and Google Ads remain separate.
- **Safe to deploy:** Yes.
...
**Next 2–4 weeks:** P2.4 Ads rebuild → PR-5, PR-7 → first Looker board one-pager → 30-day demotion-ladder step. PR-6 and PR-8 are complete.
